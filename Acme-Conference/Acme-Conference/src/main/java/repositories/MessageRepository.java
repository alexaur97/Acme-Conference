...
public interface MessageRepository extends JpaRepository<Message, Integer> {
	...
	@Query("select m from Message m where m.recipient.id=?1 and m.spam=false and m.deleted=false and m.owner.id=?1 and m.copy=true order by topic")
	...
	@Query("select m from Message m where m.sender.id=?1 and m.spam=false and m.deleted=false and m.owner.id=?1 and m.copy=false order by topic")
	...
	@Query("select m from Message m where m.recipient.id=?1 and m.spam=true and m.deleted=false and m.owner.id=?1 order by topic")
	...
	@Query("select m from Message m where m.deleted=true and m.owner.id=?1 order by topic")
